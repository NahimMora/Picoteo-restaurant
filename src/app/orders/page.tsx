  ...
  return (
    <div className="p-4 h-[calc(100vh-6rem)] md:h-[calc(100vh-9rem)] lg:px-20 xl:px-40">
      <table className="w-full border-separate border-spacing-3">
        ...
        <tbody>
          {data.map((item: OrdersType) => (
            <tr className={`${item.status !== "enviado" && 'bg-red-50'}`} key={item.id}>
              <td className="hidden md:block py-6 px-1">{item.id}</td>
              <td className="py-6 px-1">{item.createAt.toString().slice(0,10)}</td>
                ...
                <td>
                  <form className="flex items-center justify-center gap-4" onSubmit={(e) => handleUpdate(e,item.id)}>
                    ...
                    <button className="bg-red-400 p-2 rounded-full">
                      <Image src={'/edit.png'} alt="editIcon" width={20} height={20}/>
                    </button>
                  </form>
                </td>
              ) : (
